...
execution and merge contracts (O16), and a modular guard chain — LLM
firewalls and gateguards with named ingress/egress points (O17).
...
## 4. The O17 design — guard chain: LLM firewalls and gateguards
...
**registry of named ingress/egress points** that modules plug into, so
firewalls (content entering the model's context) and gateguards (actions
leaving the model's control) are additions, not surgery.

### The points

```mermaid
flowchart LR
    subgraph ingress [Ingress — content entering context]
    ...
    subgraph egress [Egress — actions and data leaving]
        EG1[EG-1 tool calls<br/>writes · shell · MCP]
        EG2[EG-2 provider calls<br/>context leaving the machine]
        EG3[EG-3 final reply + saved docs]
    end
    IN1 --> M
    IN2 --> M
    IN3 --> M
...
| Point | Today (already a guard) | What plugs in |
|---|---|---|
| IN-1 user prompt | — | prompt-injection / policy classifiers before planning |
| IN-2 tool results | `filter.js` filterToolResult (size/noise) | injection scanning on untrusted content (web pages, MCP results); provenance tagging |
| IN-3 sub-agent conclusions | `captureFromResult` shape rules | same scanners; conclusions are model-authored from untrusted inputs |
| EG-1 tool calls | jail (O1) + approval gate (O4) + denied() contract | programmatic gateguards *before* the human: path policy, command lint, secret detection in written content — the human stays the final guard |
| EG-2 provider calls | shell-env allowlist is the sibling (O6) | secret/PII scrub of outbound context; per-provider redaction policy |
| EG-3 reply + docs | — | egress policy on deliverables (secrets in generated docs/reports) |
...
4. **Order is registry order; chains short-circuit on block.** The human
   approval gate is the *last* EG-1 guard, so gateguards reduce prompts
   (auto-blocking obvious violations) rather than adding friction.
...
O16 and O17 are specified in `HARNESS_OBJECTIVES.md` (§E) and are **PLANNED**
— this document is their design record. The shipped flow in §1 is current as
of the public-release commits on `main`.
